...
class ApiRepository {
  ...
  Future<dynamic> put<T>(ApiModel<T> model) async {
    ...
    try {
      final res = await http.put(Uri.parse(model.url).replace(queryParameters: model.params), headers: model.headers, body: jsonEncode(model.body));
      if (res.statusCode == 200) {
        if (model.parse != null) {
          String data = Utf8Decoder().convert(res.bodyBytes);
          final jsonData = jsonDecode(data);
  ...
  Future<dynamic> delete<T>(ApiModel<T> model) async {
    ...
    try {
      final res = await http.delete(Uri.parse(model.url).replace(queryParameters: model.params), headers: model.headers, body: jsonEncode(model.body));
      if (res.statusCode == 200) {
        if (model.parse != null) {
          String data = Utf8Decoder().convert(res.bodyBytes);
          final jsonData = jsonDecode(data);
